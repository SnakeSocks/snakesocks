...
[[noreturn]] void Socks5Server::listen()
{
    ...
    rlog.info("Socks5 listening {}:{} ..."_format(bindIp, bindPort));
...
{ //remoteInfo.dstIp is deprecated. remoteInfo.payload contains remoteAddr, and must be processed by dns. It's a mallocd bin_str.
    ...
    if (ATYP == 1)
    {
        ...
        remoteInfo.set(bArr);
        targetPort = ntohs(*reinterpret_cast<const uint16_t *>(&pkgStr[8]));
    } else if (ATYP == 4)
    {
        ...
        remoteInfo.set(bArr);
        targetPort = ntohs(*reinterpret_cast<const uint16_t *>(&pkgStr[20]));
    } else if (ATYP == 3)
    {
        size_t addrlen = (size_t) pkgStr[4];
        targetPort = ntohs(*reinterpret_cast<const uint16_t *>(&pkgStr[5 + addrlen]));

        char *sbuffer = (char *)malloc(addrlen+1);
        rlib_defer([&]{free(sbuffer);});
...
void Socks5Connection::handshake_pkgs()
{
    ...
    sockIO::sendn_ex(m_fd, headBuf, 2);
}

Socks5Connection::addr_info Socks5Connection::connect_pkgs()
{
    char buf[263];
    sockIO::recvn_ex(m_fd, buf, 5);
    size_t addrlen = 0;
    ...
    sockIO::recvn_ex(m_fd, buf + 5, addrlen + 1);
    ...
    sockIO::sendn_ex(m_fd, buf, pkgLen);

    //remoteInfoGuarder.dismiss();
    return remoteInfo;
}

// pass dns requests to snakesocks server. dirty function design...
Socks5Connection::addr_info Socks5Connection::dns_pkgs(SnakeConnection &nextHop, addr_info &rawAddr)
{
    ...
    binary_safe_string firstPack = m_server.skserver.mod.client.encode(pkg.payload, nextHop.GetConnInfo(), pkg);
    rlib_defer([&]{std::free(firstPack.str);});
    ...
    client_query dnsResponse = m_server.skserver.mod.client.decode(firstReturnPack, nextHop.GetConnInfo());
    ...
    while (true)
    {
        ...
        if (FD_ISSET(m_fd, &rset))
        {
            ...
            if (ret == 0)
                ...
                sysdie("read failed");
            auto encodedDat = m_server.skserver.mod.client.encode(binary_safe_string{false, static_cast<uint32_t>(ret), (char *) bridgeBuffer}, nextHop.GetConnInfo(), templateQueryWithIp);
            rlib_defer([&](){std::free(encodedDat.str);});
        ...
        if (FD_ISSET(nextHop.GetConnFd(), &rset))
        {
            ...
            auto decodedDat = m_server.skserver.mod.client.decode(binary_safe_string{false, binLen, (char *) bridgeBuffer}, nextHop.GetConnInfo()).payload;
            rlib_defer([&](){free(decodedDat.str);});
